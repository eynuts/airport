...
export default function Sphere3D({
  ...
  useEffect(() => {
    ...
    if (scene.children.length === 0) {
      ...
      const auraTexCyan = createGlowTexture(
        "rgba(0, 255, 255, 1)",
        "rgba(0, 150, 255, 0.6)",
        "rgba(0, 50, 150, 0.1)",
      );
      const auraCyan = new THREE.Sprite(
      ...
      const auraTexPurple = createGlowTexture(
        "rgba(150, 100, 255, 0.8)",
        "rgba(200, 50, 255, 0.4)",
        "rgba(100, 0, 200, 0.1)",
      );
      const auraPurple = new THREE.Sprite(
        new THREE.SpriteMaterial({
          map: auraTexPurple,
          color: 0xffffff,
          transparent: true,
      ...
      const dashedGeo = new THREE.TorusGeometry(
        0.8,
        0.006,
        16,
        60,
      ...
      rings.add(
        createBgRing(1.9, 0.003, 0x0088ff, Math.PI / 1.8, -0.2, -0.001),
      );
    ...
    const animate = () => {
      ...
      if (sphereRef.current) {
        ...
        const micScaleBoost = micVolume / 800; // Subtle visual feedback for mic
        const pulse =
          baseScale + Math.sin(time * 2) * (0.01 * baseScale) + micScaleBoost;
        ...
        sphereRef.current.rotation.y +=
          (targetRotY - sphereRef.current.rotation.y) * 0.08;
        sphereRef.current.rotation.x +=
          (targetRotX - sphereRef.current.rotation.x) * 0.08;
        sphereRef.current.rotation.y += 0.002;

        if (mouthRef.current && analyserRef.current) {
          const frequency = analyserRef.current.getAverageFrequency();
          smoothFrequencyRef.current +=
            (frequency - smoothFrequencyRef.current) * 0.2;
          ...
          // Use micVolume for mouth animation when AI is not speaking
          const micMouthOpen = 1 + micVolume / 50;
          mouthRef.current.scale.y +=
            (micMouthOpen - mouthRef.current.scale.y) * 0.2;
          mouthRef.current.position.y = -0.2 - micVolume / 500;

          sphereRef.current.children.forEach((child) => {
            if (child.userData.isEye) {
              child.scale.y += (1 - micVolume / 200 - child.scale.y) * 0.1;
            }
          });
        }
        ...
        sphereRef.current.children.forEach((child) => {
          if (child.userData.isAura) {
            const micAuraBoost = micVolume / 600;
            child.material.opacity =
              0.6 + Math.sin(time * 3) * 0.2 + micVolume / 800;
            const auraPulse =
